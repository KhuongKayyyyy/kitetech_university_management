...
interface PreviewImportStudentProps {
  file: File;
  onConfirm: (validData: StudentImportData[]) => void;
  onCancel: () => void;
  isLoading?: boolean;
  open: boolean;
}

export default function PreviewImportStudent({
  file,
  onConfirm,
  onCancel,
  isLoading = false,
  open,
}: PreviewImportStudentProps) {
  const [data, setData] = useState<StudentImportData[]>([]);
  ...
  useEffect(() => {
    const parseFile = async () => {
      ...
      try {
        ...
        // Prefer the sheet named "Sample Data"; fallback to the first sheet
        const sampleSheetName =
          workbook.SheetNames.find((name) => name.trim().toLowerCase() === "student template") ||
        ...
        const parsed: StudentImportData[] = json.map((row, index) => {
